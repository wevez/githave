...
public class EntityAIControlledByPlayer extends EntityAIBase
{
    ...
    public void updateTask()
    {
        ...
        if (this.speedBoosted)
        {
            ...
            f1 += f1 * 1.15F * (float) Math.sin((float)this.speedBoostTime / (float)this.maxSpeedBoostTime * (float)Math.PI);
        }
        ...
        float f4 = (float) Math.sin(entitycreature.rotationYaw * (float)Math.PI / 180.0F);
        float f5 = (float) Math.cos(entitycreature.rotationYaw * (float)Math.PI / 180.0F);
