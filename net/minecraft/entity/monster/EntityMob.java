...
public abstract class EntityMob extends EntityCreature implements IMob
{
    ...
    public boolean attackEntityAsMob(Entity entityIn)
    {
        ...
        if (flag)
        {
            if (i > 0)
            {
                entityIn.addVelocity((double)(-(float) Math.sin(this.rotationYaw * (float)Math.PI / 180.0F) * (float)i * 0.5F), 0.1D, (double)((float) Math.cos(this.rotationYaw * (float)Math.PI / 180.0F) * (float)i * 0.5F));
                this.motionX *= 0.6D;
                this.motionZ *= 0.6D;
            }
