...
public class EntityFishHook extends Entity
{
    ...
    public EntityFishHook(World worldIn, EntityPlayer fishingPlayer)
    {
        ...
        this.posX -= (double)((float) Math.cos(this.rotationYaw / 180.0F * (float)Math.PI) * 0.16F);
        this.posY -= 0.10000000149011612D;
        this.posZ -= (double)((float) Math.sin(this.rotationYaw / 180.0F * (float)Math.PI) * 0.16F);
        this.setPosition(this.posX, this.posY, this.posZ);
        float f = 0.4F;
        this.motionX = (double)(-(float) Math.sin(this.rotationYaw / 180.0F * (float)Math.PI) * (float) Math.cos(this.rotationPitch / 180.0F * (float)Math.PI) * f);
        this.motionZ = (double)((float) Math.cos(this.rotationYaw / 180.0F * (float)Math.PI) * (float) Math.cos(this.rotationPitch / 180.0F * (float)Math.PI) * f);
        this.motionY = (double)(-(float) Math.sin(this.rotationPitch / 180.0F * (float)Math.PI) * f);
    ...
    public void handleHookCasting(double p_146035_1_, double p_146035_3_, double p_146035_5_, float p_146035_7_, float p_146035_8_)
    {
        ...
        this.prevRotationPitch = this.rotationPitch = (float)(Math.atan2(p_146035_3_, (double)f1) * 180.0D / Math.PI);
                ...
                if (!this.worldObj.isRemote && d10 > 0.0D)
                {
                    ...
                    else if (this.ticksCaughtDelay > 0)
                    {
                        ...
                        if (this.rand.nextFloat() < f1)
                        {
                            ...
                            double d12 = this.posX + (double)((float) Math.sin(f9) * f2 * 0.1F);
                            double d14 = (double)((float)MathHelper.floor_double(this.getEntityBoundingBox().minY) + 1.0F);
                            double d6 = this.posZ + (double)((float) Math.cos(f9) * f2 * 0.1F);
