...
public class EntityArrow extends Entity implements IProjectile
{
    ...
    public EntityArrow(World worldIn, EntityLivingBase shooter, float velocity)
    {
        ...
        this.motionX = (double)(-(float) Math.sin(this.rotationYaw / 180.0F * (float)Math.PI) * (float) Math.cos(this.rotationPitch / 180.0F * (float)Math.PI));
        this.motionZ = (double)((float) Math.cos(this.rotationYaw / 180.0F * (float)Math.PI) * (float) Math.cos(this.rotationPitch / 180.0F * (float)Math.PI));
        this.motionY = (double)(-(float) Math.sin(this.rotationPitch / 180.0F * (float)Math.PI));
    ...
    public void onUpdate()
    {
        ...
        else
        {
            ...
            for (this.rotationPitch = (float)(Math.atan2(this.motionY, (double)f3) * 180.0D / Math.PI); this.rotationPitch - this.prevRotationPitch < -180.0F; this.prevRotationPitch -= 360.0F)
            {
                ;
            }
