...
public abstract class WorldProvider
{
    ...
    {
        float f = 0.4F;
        float f1 = (float) Math.cos(celestialAngle * (float)Math.PI * 2.0F) - 0.0F;
        float f2 = -0.0F;

        if (f1 >= f2 - f && f1 <= f2 + f)
        {
            float f3 = (f1 - f2) / f * 0.5F + 0.5F;
            float f4 = 1.0F - (1.0F - (float) Math.sin(f3 * (float)Math.PI)) * 0.99F;
            f4 = f4 * f4;
    ...
    {
        float f = (float) Math.cos(p_76562_1_ * (float)Math.PI * 2.0F) * 2.0F + 0.5F;
