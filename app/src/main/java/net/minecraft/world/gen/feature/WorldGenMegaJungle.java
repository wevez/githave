    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        else
        {
            ...
            for (int j = position.getY() + i - 2 - rand.nextInt(4); j > position.getY() + i / 2; j -= 2 + rand.nextInt(4))
            {
                ...
                int k = position.getX() + (int)(0.5F + (float) Math.cos(f) * 4.0F);
                int l = position.getZ() + (int)(0.5F + (float) Math.sin(f) * 4.0F);

                for (int i1 = 0; i1 < 5; ++i1)
                {
                    k = position.getX() + (int)(1.5F + (float) Math.cos(f) * (float)i1);
                    l = position.getZ() + (int)(1.5F + (float) Math.sin(f) * (float)i1);
