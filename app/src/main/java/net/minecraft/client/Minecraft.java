...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    private void createDisplay() throws LWJGLException
    {
        ...
        Display.setTitle("Loading " + GitHave.INSTANCE.name + " " + GitHave.INSTANCE.version);
    ...
    private void runGameLoop() throws IOException
    {
        ...
        {
            GitHave.INSTANCE.eventManager.call(timeDelay);
        }
        ...
        if (this.mcProfiler.profilingEnabled)
        {
            ...
            for (int l = 0; l < list.size(); ++l)
            {
                ...
                for (int j2 = i1; j2 >= 0; --j2)
                {
                    ...
                    float f1 = (float) Math.sin(f) * (float)i;
                    float f2 = (float) Math.cos(f) * (float)i * 0.5F;
                ...
                for (int i3 = i1; i3 >= 0; --i3)
                {
                    ...
                    float f4 = (float) Math.sin(f3) * (float)i;
                    float f5 = (float) Math.cos(f3) * (float)i * 0.5F;
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput)
        {
            ...
            while (Keyboard.next())
            {
                ...
                if (Keyboard.getEventKeyState())
                {
                    ...
                    {
                        GitHave.INSTANCE.moduleManager.getModules().stream().filter(m -> m.getKeyCode() == k).forEach(Module::toggle);
                        if (k == Keyboard.KEY_RSHIFT) {
                            this.displayGuiScreen(GitHave.INSTANCE.clickGui);
                        }
