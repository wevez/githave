...
public class ModelWitch extends ModelVillager
{
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn)
    {
        ...
        this.villagerNose.rotateAngleX = (float) Math.sin((float)entityIn.ticksExisted * f) * 4.5F * (float)Math.PI / 180.0F;
        this.villagerNose.rotateAngleY = 0.0F;
        this.villagerNose.rotateAngleZ = (float) Math.cos((float)entityIn.ticksExisted * f) * 2.5F * (float)Math.PI / 180.0F;
