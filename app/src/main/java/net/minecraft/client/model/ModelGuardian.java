...
public class ModelGuardian extends ModelBase
{
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn)
    {
        ...
        for (int i = 0; i < 12; ++i)
        {
            ...
            this.guardianSpines[i].rotationPointX = afloat3[i] * (1.0F + (float) Math.cos(ageInTicks * 1.5F + (float)i) * 0.01F - f1);
            this.guardianSpines[i].rotationPointY = 16.0F + afloat4[i] * (1.0F + (float) Math.cos(ageInTicks * 1.5F + (float)i) * 0.01F - f1);
            this.guardianSpines[i].rotationPointZ = afloat5[i] * (1.0F + (float) Math.cos(ageInTicks * 1.5F + (float)i) * 0.01F - f1);
        }
        ...
        this.guardianTail[0].rotateAngleY = (float) Math.sin(f2) * (float)Math.PI * 0.05F;
        this.guardianTail[1].rotateAngleY = (float) Math.sin(f2) * (float)Math.PI * 0.1F;
        this.guardianTail[1].rotationPointX = -1.5F;
        this.guardianTail[1].rotationPointY = 0.5F;
        this.guardianTail[1].rotationPointZ = 14.0F;
        this.guardianTail[2].rotateAngleY = (float) Math.sin(f2) * (float)Math.PI * 0.15F;
