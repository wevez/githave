    ...
    public void onLivingUpdate()
    {
        ...
        if (this.inWater)
        {
            if (this.squidRotation < (float)Math.PI)
            {
                ...
                this.tentacleAngle = (float) Math.sin(f * f * (float)Math.PI) * (float)Math.PI * 0.25F;
            ...
            this.squidPitch += (-((float)Math.atan2((double)f1, this.motionY)) * 180.0F / (float)Math.PI - this.squidPitch) * 0.1F;
        }
        else
        {
            this.tentacleAngle = Math.abs((float) Math.sin(this.squidRotation)) * (float)Math.PI * 0.25F;
    ...
    static class AIMoveRandom extends EntityAIBase
    {
        ...
        public void updateTask()
        {
            ...
            else if (this.squid.getRNG().nextInt(50) == 0 || !this.squid.inWater || !this.squid.func_175567_n())
            {
                ...
                float f1 = (float) Math.cos(f) * 0.2F;
                float f2 = -0.1F + this.squid.getRNG().nextFloat() * 0.2F;
                float f3 = (float) Math.sin(f) * 0.2F;
