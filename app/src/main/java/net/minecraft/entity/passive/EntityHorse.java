...
public class EntityHorse extends EntityAnimal implements IInvBasic
{
    ...
    public void moveEntityWithHeading(float strafe, float forward)
    {
        if (this.riddenByEntity != null && this.riddenByEntity instanceof EntityLivingBase && this.isHorseSaddled())
        {
            ...
            if (this.jumpPower > 0.0F && !this.isHorseJumping() && this.onGround)
            {
                ...
                if (forward > 0.0F)
                {
                    float f = (float) Math.sin(this.rotationYaw * (float)Math.PI / 180.0F);
                    float f1 = (float) Math.cos(this.rotationYaw * (float)Math.PI / 180.0F);
    ...
    public void updateRiderPosition()
    {
        ...
        {
            float f = (float) Math.sin(this.renderYawOffset * (float)Math.PI / 180.0F);
            float f1 = (float) Math.cos(this.renderYawOffset * (float)Math.PI / 180.0F);
