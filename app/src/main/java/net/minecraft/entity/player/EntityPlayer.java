    ...
    public void attackTargetEntityWithCurrentItem(final Entity targetEntity) {
        if (targetEntity.canAttackWithItem() && !targetEntity.hitByEntity(this)) {
            ...
            if (f > 0.0f || f2 > 0.0f) {
                ...
                if (flag3) {
                    Events.Knockback eventAttackSlowdown = new Events.Knockback(0.6, false, 1, 0, false, false);
                    GitHave.INSTANCE.eventManager.call(eventAttackSlowdown);
                    if (i > 0) {
                        if(!eventAttackSlowdown.isCanceled()) {
                            for(int power = 0; power < eventAttackSlowdown.power; power++) {
                                targetEntity.addVelocity(-Math.sin(this.rotationYaw * 3.1415927f / 180.0f) * i * 0.5f, eventAttackSlowdown.reduceY ? 0 : 0.1, Math.cos(this.rotationYaw * 3.1415927f / 180.0f) * i * 0.5f);
                            }
                            this.motionX *= eventAttackSlowdown.motion;
                            this.motionZ *= eventAttackSlowdown.motion;
                            this.setSprinting(false);
                        }
                    }
                    else if (eventAttackSlowdown.full && Minecraft.getMinecraft().thePlayer.hurtTime > 0) {
                        for(int power = 0; power < eventAttackSlowdown.postPower; power++) {
                            targetEntity.addVelocity(-Math.sin(this.rotationYaw * 3.1415927f / 180.0f) * i * 0.5f, eventAttackSlowdown.reduceY ? 0 : 0.1, Math.cos(this.rotationYaw * 3.1415927f / 180.0f) * i * 0.5f);
                        }
                        if(!eventAttackSlowdown.isCanceled()) {
                            this.motionX *= eventAttackSlowdown.motion;
